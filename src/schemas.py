...
class AnswerResponse(BaseModel):
    ...
    sources: List[str] = Field(
        ...
        description="Source document IDs used to produce the answer",
    )
...
class UserIntent(BaseModel):
    ...
    intent_type: Literal["qa", "summarization", "calculation", "unknown"] = Field(
        description="Classified intent: qa, summarization, calculation, or unknown",
    )
    confidence: float = Field(
        ge=0.0,
        le=1.0,
        description="Confidence in classification between 0 and 1",
    )
